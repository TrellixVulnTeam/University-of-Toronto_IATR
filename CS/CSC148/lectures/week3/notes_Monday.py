...
class Rational:
    ...
    # in Python, we use a leading underscore to denote
    # "private"
    # this is a convention, it doesnot change the way
    # the code works, just how programmer should work with it

    def _get_denominator(self):
        """"""
        return self._denom

    def _set_denominator(self, new_denom):
        """"""
        # raise Exception
        if type(new_denom) != int:
            raise Exception('Denominator must be int')
        if new_denom == 0:
            raise Exception('Denominator cannot be zero')

        self._denom = new_denom

    denom = property(_get_denominator, _set_denominator)
...
if __name__ == '__main__':
    if __name__ == '__main__':
        ...
        for rational in rational_list:
            print(rational)
        r3.denom = 7
        #r4.denom = 0 #this led error
        r4.set_denominator(0)
        print(r4.to_float())
        #ideally, we'd likt to be able to access out attributes
        #r.num... r.deno...
        #but, also get the protection of setters
