...
HuffmanNode(None, HuffmanNode(5, None, None), HuffmanNode(7, None, None)))
    """
    #if node_lst[root_index].l_type == 0 and node_lst[ro
    if node_lst[root_index].l_type == 0 and node_lst[root_index].r_type == 0:
        ...
        None), HuffmanNode(node_lst[root_index].r_data, None, None))
    elif node_lst[root_index].l_type == 0 and node_lst[root_index].r_type == 1:
        return HuffmanNode(None, HuffmanNode(node_lst[root_index].l_data, None,
        None), generate_tree_general(node_lst, node_lst[root_index].r_data))
    elif node_lst[root_index].l_type == 1 and node_lst[root_index].r_type == 0:
        return HuffmanNode(None, generate_tree_general(node_lst, node_lst
        [root_index].l_data), HuffmanNode(node_lst[root_index].r_data,
        None, None))
    else:
        return HuffmanNode(None, generate_tree_general(node_lst, node_lst
...
HuffmanNode(None, HuffmanNode(10, None, None), HuffmanNode(12, None, None)))
    ...
    if node_lst[root_index].l_type == 0 and node_lst[root_index].r_type == 0:
        ...
        return HuffmanNode(None, HuffmanNode(node_lst[root_index].l_data,
               None, None), generate_tree_postorder(node_lst, root_index - 1))
    elif node_lst[root_index].l_type == 1 and node_lst[root_index].r_type == 0:
        return HuffmanNode(None, generate_tree_postorder(node_lst, root_index
               - 1), HuffmanNode(node_lst[root_index].r_data, None, None))
    else:
        return HuffmanNode(None, generate_tree_postorder(node_lst, root_index -
               len(get_node(generate_tree_general(node_lst, root_index - 1))) -
               1), generate_tree_general(node_lst, root_index - 1))
...
def generate_uncompressed(tree, text, size):
    ...
    number = 0
    text_to_read = ''.join([byte_to_bits(item) for item in text])
    while begin < len(text_to_read) and new_str not in inverse_dict and \
                    number < size:
        i = 1
        while begin + i <= len(text_to_read) and new_str not in inverse_dict:
            new_str = text_to_read[begin: begin + i]
