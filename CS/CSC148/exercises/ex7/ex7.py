...
def list_longest_path(node):
    ...
    if node is None:
        ...
        l = list_longest_path(node.left)
        r = list_longest_path(node.right)
        if len(l) > len(r):
            return [node.value] + l
        else:
            return [node.value] + r
...
def list_between(node, start, end):
    ...
    if node is None:
        ...
        if start <= node.value <= end:
            return list_between(node.left, start, end) + [node.value] + \
...
def count_shallower(t, n):
    ...
    def act(node, d, limit):
        """Return a depth list of nodes that is under limit depth.

        @param BinaryTree|None node: binary tree to count
        @param int d: depth
        @param int limit: limit depth
        @rtype: list
        """
        if d == limit - 1:
            return [d]
            ...
            if node.left is not None and node.right is not None:
                return act(node.left, d + 1, limit) + act(node.right, d + 1,
                                                          limit) + \
                       [0]

            elif node.left is not None and node.right is None:
                return act(node.left, d + 1, limit) + [0]

            elif node.right is not None and node.left is None:
                return act(node.right, d + 1, limit) + [0]
            else:
                return [0]
    if t is None:
        return 0
