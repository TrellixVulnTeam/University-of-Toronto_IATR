...
int accept_connection(int fd, struct sockname *connections) {
    ...
    int client_fd = accept(fd, NULL, NULL);//做小张
    if (client_fd < 0) {
    ...
    char buf[BUF_SIZE+1];
    int num_read = read(client_fd,buf,BUF_SIZE-1); //-1 ? -2 ?
    buf[num_read-1] = '\0';
    strcat(buf, ": ");
    ...
    connections[user_index].username = malloc(strlen(buf)+1);
...
int read_from(int client_index, struct sockname *connections) {
    ...
    if (num_read == 0 || write(fd, buf, strlen(buf)) != strlen(buf)) {//被关了
...
int main(void) {
    ...
    while (1) {
        ...
        if (FD_ISSET(sock_fd, &listen_fds)) {
            int client_fd = accept_connection(sock_fd, connections);//实际接待业务的socket！
            if (client_fd > max_fd) {
                max_fd = client_fd;
            }
            FD_SET(client_fd, &all_fds);//加到 watch-list
