...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < iterations+1; i++) {
    ...
    if (n < 0) {
      ...
      //read the m value from pipe_i
      int m;
      if (read(fd[i][0], &m, sizeof(int)) == -1) {
        perror("reading from pipe from pipe");
      }

      if (m >= sqrt(iterations)){
        if (m * m != iterations){
        // printf("%d is greater than or equal to the square root of iterations\n", m);
        //no factor
        if (factor_counter == 0){
          printf("%d is prime\n", iterations);
        }
        // exact one factor
          ...
          int another_prime = iterations / factor_1;
          if (another_prime % factor_1 == 0){
            printf("%d is not the product of two primes\n", iterations );
          }else{
            printf("%d %d %d\n", iterations, factor_1, another_prime);
          }
        }
        exit(0);
        }else{
      ...
      if (iterations % m == 0){//m is a factor
          if (factor_counter == 0){
            factor_1 = m;
            factor_counter +=1;
          }else{
