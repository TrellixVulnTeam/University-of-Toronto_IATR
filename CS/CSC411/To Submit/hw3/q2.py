...
def run_validation(x, y, taus, val_frac):
    ...
    for tau in taus:
        predicted_y_test_array = []
        for test_datum in X_test:
            predicted_y = LRLS(test_datum.reshape((1, d)), X_train, Y_train, tau, lam=1e-5)
            predicted_y_test_array.append(predicted_y)

        validation_cost = np.sum((np.array(predicted_y_test_array) - Y_test) ** 2) / (2. * len(Y_test))
        validation_loss.append(validation_cost)

        predicted_y_train_array = []
        for test_datum in X_train:
            predicted_y_2 = LRLS(test_datum.reshape((1, d)), X_train, Y_train, tau, lam=1e-5)
            predicted_y_train_array.append(predicted_y_2)

        train_cost = np.sum((np.array(predicted_y_train_array) - Y_train) ** 2) / (2. * len(Y_train))
...
if __name__ == "__main__":
    ...
    plt.plot(taus, train_losses)
    plt.ylabel("Loss")
    plt.xlabel("log taus")
    plt.title("Blue line: validation loss Orange line: training loss")
    plt.semilogx()
    #plt.show()
    plt.savefig('train losses & validation losses.png')