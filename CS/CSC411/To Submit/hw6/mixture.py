...
class Model(object):
    ...
    def update_pi(self, R):
        ...
        k = R.shape[1]
        numerator = self.prior.a_mix - 1 + np.sum(R, axis=0)
        denominator = k * self.prior.a_mix - k + np.sum(R)
    ...
    def update_theta(self, X, R):
        ...
        numerator = R.transpose()@X + a - 1
        denominator = np.sum(R, axis=0).reshape(-1, 1) + a + b - 2
        return numerator / denominator
    ...
    def compute_posterior(self, X, M=None):
        ...
        ######################## Your code here #########################
        log_p_z_x = (M * X) @ np.log(self.params.theta).T + \
                    (M * (1 - X)) @ np.log(1 - self.params.theta).T + np.log(self.params.pi)
    ...
    def posterior_predictive_means(self, X, M):
        ...
        return np.dot(R, self.params.theta)
