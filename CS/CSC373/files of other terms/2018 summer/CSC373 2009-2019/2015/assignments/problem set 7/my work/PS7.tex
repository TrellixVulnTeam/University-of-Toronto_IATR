...
It is a verifier for $\const{Frugal}$ because $\proc{Frugal-Verifier}(G, R') = \const{true}$ for some $R'\subseteq R$ if and only if there is some subset of recipes $R' \subseteq R$
        with size $|R'| \le M$ such that all together,
        the recipes in $R'$ use up exactly the ingredients from $G$.
