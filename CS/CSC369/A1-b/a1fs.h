...
typedef struct a1fs_superblock {
	...
	unsigned int inode_bitmap;      /* Inodes bitmap starting block */
	unsigned int block_bitmap;      /* Blocks bitmap starting block */
	unsigned int inode_table;    /*inode table staring block*/
	unsigned int data_blocks_str;    /*read staring block of real data blocks*/

	a1fs_blk_t inodes_count; /* number of inode*/
	a1fs_blk_t blocks_count; /* number of block of fs*/
	a1fs_blk_t free_blocks_count; /* number of free data block*/
	a1fs_blk_t free_inodes_count; /* number of free inode*/
	a1fs_blk_t inode_bitmap_blocks_count; /* number of blocks of inode_bitmap */
	a1fs_blk_t block_bitmap_blocks_count; /* number of blocks of block_bitmap */
	a1fs_blk_t inode_table_blocks_count; /* number of blocks of inode_table */
} a1fs_superblock;
...
/** a1fs inode. 
 * size of inode struct is 64 byte in below definition
*/
typedef struct a1fs_inode {
	...
	// block number of the extent table for this file
	a1fs_blk_t extent_table;

	// number of extents of this file
	a1fs_blk_t extents_count;

	// if it is dir, entry count
	int entry_count;
