...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in nxtvar.curDomain():
        ...
        nxtvar.restoreValues(nxtvar, val)
    #undo assignment to var
    nxtvar.unAssign()
    #restore var to unAssignedVars
    unAssignedVars.insert(nxtvar)
    return solns
...
def GAC(unAssignedVars, csp, allSolutions, trace):
    ...
    for value in var.curDomain():
        ...
        if GacEnforce(csp.constraintsOf(var), csp, var, value) == "DWO":
            noDwo = False
        if noDwo:
            nxt = GAC(unAssignedVars, csp, allSolutions, trace)
            sols.extend(nxt)

            if len(sols) > 0 and not allSolutions:
                var.restoreValues(var, value)
                break
        var.restoreValues(var, value)
    var.setValue(None)
