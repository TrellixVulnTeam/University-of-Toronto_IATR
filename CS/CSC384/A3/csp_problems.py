...
def sudokuCSP(initial_sudoku_board, model='neq'):
    ...
    for row in var_array:
        if model == 'neq':
            ...
            constraint_list.append (AllDiffConstraint(name="Row", scope=row))
    ...
    for colj in range(len(var_array[0])):
        ...
        if model == 'neq':
            ...
            constraint_list.append (AllDiffConstraint(name="Column", scope=scope))
    ...
    for i in [0, 3, 6]:
        for j in [0, 3, 6]:
            ...
            if model == 'neq':
                ...
                constraint_list.append (AllDiffConstraint(name="Subsquare", scope=scope))
...
def solve_planes(planes_problem, algo, allsolns,
    ...
    #initialize valid orders
    valid_orders = []
    for flight_1, flight_2 in planes_problem.can_follow:
        valid_orders.append([flight_1, flight_2])

    for flight in planes_problem.flights:
        valid_orders.append([flight, 'empty'])

    valid_orders.append(['empty', 'empty'])

    maintenance_flights = planes_problem.maintenance_flights + ['empty']

    for plane in planes_problem.planes:
        #get can-fly flights for this plane
        flight_pairs = []
        domain = planes_problem.can_fly(plane) + ['empty']

        #create variable and add to variable list
        var_start = Variable(plane + '_0', planes_problem.can_start(plane) + ['empty'])

        variables.append(var_start)
        flight_pairs.append(var_start)

        for i in range(1, len(domain)-1):
            var_name =  plane + "_{}".format(i)
            var = Variable(var_name, domain)

            variables.append(var)
            flight_pairs.append(var)


        # add flight maintenance constraint
        if len(flight_pairs) >= planes_problem.min_maintenance_frequency:
            for i in range(0, len(flight_pairs) - planes_problem.min_maintenance_frequency + 1):
                temp = flight_pairs[i: i + planes_problem.min_maintenance_frequency]
                temp_name = plane + "_maintenance_{}".format(i)
                NValuesConstraint_instance = NValuesConstraint(temp_name, temp, maintenance_flights, 1, planes_problem.min_maintenance_frequency)
                constraints.append(NValuesConstraint_instance)

        # add order constraints 
        for i in range(0, len(flight_pairs) - 1):
            flight_1 = flight_pairs[i]
            flight_2 = flight_pairs[i + 1]
            table_constraint_instance = TableConstraint(plane + '_flight_order_{}'.format(i), [flight_1, flight_2], valid_orders)
            constraints.append(table_constraint_instance)

        
    # add one and exact one flight constraint
    for flight in planes_problem.flights:
        constraints.append(NValuesConstraint('exact one flight', variables, [flight], 1, 1))
    ...
    for item in solutions:
        ...
        for tpl in item:
            var = tpl[0]
            flight = tpl[1]
            #if flight is not empty, otherwise skip it
            if flight != 'empty': 
                temp_lst = var.name().split('_')
                plane = temp_lst[0]

                if plane not in sol:
                    sol[plane] = [flight]
                else:
                    sol[plane].append(flight)
        
        #convert to expected lst
        new_sol = [[plane] + [flight for flight in sol[plane]] for plane in sol]
        res.append(new_sol)
    # print(res)
    return res    