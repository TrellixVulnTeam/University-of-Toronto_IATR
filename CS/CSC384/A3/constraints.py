...
class QueensTableConstraint(TableConstraint):
    ...
    def __init__(self, name, qi, qj, i, j):
        ...
        if i == j:
            TableConstraint.__init__(self, name, [qi, qj], satisfied_assignments)
        else:
            for i_value in qi.curDomain():
                for j_value in qj.curDomain():
                    if (abs(i_value - j_value) != abs(i - j)) and (i_value != j_value):
                        satisfied_assignments.append([i_value, j_value])
            TableConstraint.__init__(self, name, [qi, qj], satisfied_assignments)
    ...
    def check(self):
        ...
        for v in self.scope():
            if v.getValue() in self._required:
                count += 1
        part_one = self._lb <= count
        part_two = count <= self._ub
        return part_one and part_two
